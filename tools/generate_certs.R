...
generate_certs <- function(
  ...
  if (!nzchar(mkcert_path) && !nzchar(openssl_path)) {
    stop(
      "Neither mkcert nor openssl found in PATH. Please install one of them."
    )
  }
  ...
  if (!force && (file.exists(cert_file) || file.exists(key_file))) {
    stop(
      "Certificate or key file already exists in ",
      dir,
    ...
    tryCatch(
      {
        ...
        if (file.exists(cert_file) && file.exists(key_file)) {
          message(
            ...
            "  Certificate: ",
            normalizePath(cert_file),
            "\n",
            "  Key: ",
            normalizePath(key_file),
            "\n",
