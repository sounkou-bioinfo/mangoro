...
This demonstrates thread-based concurrency where the Go process runs both an RPC server (REQ/REP pattern) for control commands and HTTP server (standard Go net/http) running in a goroutine


The `rgoipc` package provides interfaces for type-safe function registration with Arrow schema validation. See [inst/go/pkg/rgoipc](inst/go/pkg/rgoipc) for the Go package implementation and [inst/go/cmd/rpc-example](inst/go/cmd/rpc-example) and [inst/go/cmd/http-server](inst/go/cmd/http-server) for complete server examples.
