...
The package includes an HTTP file server that can be controlled via RPC, demonstrating a more complex use case. Unlike the CGo+pipes approach, this uses the nanomsg/nanonext IPC framework for all communication.
...
This demonstrates thread-based concurrency where the Go process runs both:
1. An RPC server (REQ/REP pattern) for control commands
2. An HTTP server (standard Go net/http) running in a goroutine

No CGo or pipes are needed - all communication happens via nanomsg IPC sockets.
