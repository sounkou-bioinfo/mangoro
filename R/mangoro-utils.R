...
mangoro_rpc_parse_response <- function(response) {
    ...
    # Handle NA from unpacking
    if (is.na(error_len)) error_len <- 0L
...
#' @export
mangoro_http_start <- function(sock, addr, dir = ".", prefix = "/",
                               cors = FALSE, coop = FALSE, tls = FALSE, silent = FALSE) {
    input_df <- data.frame(
        addr = addr,
        dir = dir,
        prefix = prefix,
